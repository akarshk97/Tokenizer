...
def readText(numdocs):
    ...
    #print("current file", counter)
        counter = counter + 1
        prefix = i.split(".")
        path = "/Users/akarshkashamshetty/OneDrive - UMBC/grad_sem4/CMSC676/KashamshettyAkarshHW3/" + inputDir + i
...
def writeFiles(dict1, dict2):
    with open(outputDir+'dictionaryFile.txt', 'w') as f:
        for k in dict1.keys():
            f.write('{}\n'.format(k))
            f.write('{}\n'.format(dict1[k][0]))
            f.write('{}\n'.format(dict1[k][1]))
    with open(outputDir+'postingsFile.txt','w') as ff:
